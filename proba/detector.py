# Vendored from mihail-gribov/quadrat-ipi-eval (Apache-2.0), method code unchanged.
# ONLY DEVIATION: the adapter loader imported `quadrat.detectors.*` by hardcoded name;
# it now uses `__package__` so the loader works under this package's name. No behaviour change.
...
def load_detectors(extra_dirs=()) -> None:
    ...
    `proba/detectors/` holds the adapters the published table was measured with. `extra_dirs`
    ...
    for m in pkgutil.iter_modules([str(pkg)]):
        if not m.name.startswith("_"):
            importlib.import_module(f"{__package__}.detectors.{m.name}")
    ...
    for d in extra_dirs:
        ...
        for f in sorted(d.glob("*.py")):
            if f.name.startswith("_"):
                continue
            spec = importlib.util.spec_from_file_location(f"proba_adapter_{f.stem}", f)
